...
| --- | --- | --- |
| Seeed Studio reTerminal Sticky | **v1.0.18** | Production target; v1.0.18 restores reliable short-press wake and changes GT911 deep-sleep handling for wake reliability |
...
v1.0.18 isolates the remaining problem to the GT911 touchscreen and does not alter the board power latch.

The important change is that **GPIO42 / GT911 is kept powered through ESP32 deep sleep instead of being cold-power-cycled on every sleep/wake transition**. The v1.0.14 base shutdown briefly requests the touch rail off; the v1.0.18 shutdown hook runs afterward and restores GPIO42 HIGH, then holds that HIGH level through deep sleep. The normal GT911 reset-pin initialization still runs on boot.

Additional GT911 startup hardening:

- the dedicated touch I²C bus no longer performs an unnecessary startup scan;
...
- known raw dimensions are supplied as calibration (`480×800`), avoiding an additional startup calibration-register read that can cause ESPHome's current GT911 driver to mark the touchscreen failed.
...
The deep-sleep TV-on recovery path still waits for Home Assistant to reconnect and then invokes `script.tv_turn_on_the_tv_cable`, the same TV-on sequence used by Alexa.
...
## v1.0.18 validation checklist

1. Install v1.0.18 while the Sticky is awake.
2. Confirm awake touchscreen controls work before the first sleep cycle.
3. Turn the TV off and allow the remote to render the sleep screen and enter deep sleep.
4. Wake with a **brief tap** of AI / Power; do not hold it.
5. Confirm the Sticky stays powered and `script.tv_turn_on_the_tv_cable` turns the TV on.
6. Immediately touch several D-pad buttons and confirm **Last Touch X/Y** updates in Home Assistant.
7. Confirm Back, Home, playback, app launchers, and both physical volume buttons work.
8. Repeat the TV-off → sleep → brief-tap wake cycle at least five times and verify touch works after every wake.
9. Confirm battery telemetry remains plausible and observe sleep-current/battery-life impact before further power optimization.
